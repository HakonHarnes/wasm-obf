  ...
  execute() {
    ...
    if (
      ...
      switch (this.operator) {
        case "add":
          newType = this.typeResolver(
            this.lValue.dataType,
            this.rValue.dataType
          );
          return new ImmediateValue(
            this.lValue.value + this.rValue.value,
            newType,
            this.lineNumberSet
          );
        case "sub":
        ...
        case "div_u": // NEED TO FIX THIS
          ...
          return new ImmediateValue(
            this.getUnsignedInt(this.lValue.value) /
              this.getUnsignedInt(this.rValue.value),
            newType,
            this.lineNumberSet
          );
        case "shl":
          ...
          return new ImmediateValue(
            this.rValue.value << this.lValue.value,
            newType,
            this.lineNumberSet
          );
        case "shr_s":
...
class StoreAbstraction extends Abstraction {
  ...
  canMerge(otherAbstraction) {
    ...
    if (otherAbstraction.AbstractionType == "STORE") {
      ...
      if (
        this.MemoryLocation.isIMM() &&
        otherAbstraction.MemoryLocation.isIMM()
      ) {
        if (
          Math.abs(
            this.MemoryLocation.Value - otherAbstraction.MemoryLocation.Value <=
              8
          )
        ) {
          return true;
      ...
      if (
        ...
        (this.Offset == null &&
          [1, 2, 4, 8].includes(otherAbstraction.Offset)) ||
        ([1, 2, 4, 8].includes(this.Offset) && otherAbstraction.Offset == null)
      ) {
        if (
          lodash.isEqual(this.MemoryLocation, otherAbstraction.MemoryLocation)
        ) {
          return true;
        } else {
          return false;
      ...
      const possibleAdjacentItem =
        otherAbstraction.InnerAbstractions[
          otherAbstraction.InnerAbstractions.length - 1
        ];

      if (
        this.MemoryLocation.isIMM() &&
        possibleAdjacentItem.MemoryLocation.isIMM()
      ) {
        if (
          Math.abs(
            this.MemoryLocation.Value -
              possibleAdjacentItem.MemoryLocation.Value <=
              8
          )
        ) {
          return true;
      ...
      if (
        (this.Offset == null &&
          [1, 2, 4, 8].includes(possibleAdjacentItem.Offset)) ||
        ([1, 2, 4, 8].includes(this.Offset) &&
          possibleAdjacentItem.Offset == null)
      ) {
        if (
          lodash.isEqual(
            this.MemoryLocation,
            possibleAdjacentItem.MemoryLocation
          )
        ) {
          return true;
        }
...
class ForAbstraction extends Abstraction {
  ...
  Dec_Expr() {
    ...
    while (
      !innerAbstractionsToSearch.isEmpty() &&
      this.DecrementExpression == null
    ) {
...
class MemCpyAbstraction extends Abstraction {
  ...
  UpdateWithCallArguments() {
    ...
    if (callArguments != null) {
      ...
      this.InnerAbstractions.forEach((abstr) => {
        ...
        abstr.MemoryLocation =
          abstr.MemoryLocation.updateReference(callArguments);
      });
    }
  }
  ...
  IsMemSet() {
    if (this.InnerAbstractions.length < 2) {
      ...
      const callArguments = this.AbstractionCallArguments();
      if (callArguments != null) {
        this.InnerAbstractions[0].Value =
          this.InnerAbstractions[0].Value.updateReference(callArguments);
        if (this.InnerAbstractions[0].Value.isOP()) {
          this.InnerAbstractions[0].Value =
            this.InnerAbstractions[0].Value.execute();
          if (this.InnerAbstractions[0].Value.isIMM()) {
            return true;
          }
        }
...
class VirtualState {
  ...
  getLocalVariable(varName) {
    if (!this.LocalVariables[varName]) {
      ...
      const newVarReference = new VariableReference(
        newSymbolicValue,
        lineNumber
      );
...
class VirtualStateManager {
  ...
  moveContextToElse() {
    this.moveContextUp();
    const ifAbstraction =
      this.CurrentAbstractionList[this.CurrentAbstractionList.length - 1];
    this.AbstractionStack.push(this.CurrentAbstractionList);
  ...
  moveContextUp() {
    ...
    if (this.AbstractionStack.length > 0) {
      const previousAbstractionLevel =
        this.AbstractionStack[this.AbstractionStack.length - 1];
      this.UpperAbstraction =
        previousAbstractionLevel[previousAbstractionLevel.length - 1];
    } else {
      this.UpperAbstraction = null;
    }

    const lastContextedAbstraction =
      this.CurrentAbstractionList[this.CurrentAbstractionList.length - 1];
  ...
  handleSetLocal(token, nextToken) {
    this.VirtualStates[this.VirtualStates.length - 1].forEach(
      (activeState, index) => {
        const varName = nextToken;
  ...
  handleLoad(token, nextToken, secondToken) {
    this.VirtualStates[this.VirtualStates.length - 1].forEach((activeState) => {
      ...
      const newMemoryRef = new MemoryReference(
        location,
        align,
        offset,
        lineNumber
  ...
  handleStore(token, nextToken, secondToken) {
    this.VirtualStates[this.VirtualStates.length - 1].forEach(
      (activeState, index) => {
        ...
        if (index == 0) {
          const newStoreAbstraction = new StoreAbstraction(
            location,
            valueToStore,
            align,
            offset,
          ...
          if (
            previousAbstraction != null &&
            newStoreAbstraction.canMerge(previousAbstraction)
          ) {
            previousAbstraction =
              previousAbstraction.merge(newStoreAbstraction);
        ...
        if (location.isIMM()) {
          ...
          const newSymbolicMemoryStore = new MemoryReference(
            location,
            align,
            offset,
            lineNumber
          ...
          activeState.LinearMemory.SymbolicLocations.push(
            newSymbolicMemoryStore
          );
        }
      },
      this
  ...
  handleIf() {
    ...
    const conditionDetails =
      this.VirtualStates[this.VirtualStates.length - 1][0].pop();
  ...
  handleCall(functionName) {
    this.VirtualStates[this.VirtualStates.length - 1].forEach(
      (activeState, index) => {
        ...
        if (index == 0) {
          const newCallAbstraction = new CallAbstraction(
            functionName,
            namedArgs
          );
    ...
    this.VirtualStates[this.VirtualStates.length - 1].forEach(
      (activeState, index) => {
        ...
        if (functionType != null && functionType.Params != null) {
          ...
          if (functionType.Result != null) {
            resultVar = new SymbolicValue(
              "call_indirect" + type + "_result",
              lineNumber
            );
          }
          if (resultVar) {
        ...
        if (index == 0) {
          ...
          newCallIndirectAbstraction.Paths = Array.from(
            new Set(ProgramDetails.TableFunctions)
          );
...
class Graph {
  ...
  addEdge(prevAbstraction, nextAbstraction) {
    this.AdjacencyList.get(prevAbstraction).AdjacentVertices.push(
      nextAbstraction
    );
  }
  ...
  *traverseBFS(startingNode = this.FirstVertex, traverseInner = false) {
    ...
    if (startingNode.ParentAbstraction != null) {
      if (
        ...
        const abstrIndexInParent =
          startingNode.ParentAbstraction.InnerAbstractions.indexOf(
            startingNode
          );

        for (
          let i = abstrIndexInParent;
          i < startingNode.ParentAbstraction.InnerAbstractions.length;
          i++
        ) {
    ...
    while (!nodesToVisit.isEmpty()) {
      ...
      if (traverseInner == true) {
        if (
          abstraction.AbstractionType == "FOR" ||
          abstraction.AbstractionType == "BLOCK"
        ) {
  ...
  *traverseDFSArrayOnly(startingNode = this.FirstVertex) {
    ...
    while (midpointNode.ParentAbstraction != null) {
      const abstrIndexInParent =
        midpointNode.ParentAbstraction.InnerAbstractions.indexOf(midpointNode);

      for (
        let i = abstrIndexInParent;
        i < midpointNode.ParentAbstraction.InnerAbstractions.length;
        i++
      ) {
...
class DatabaseConnector {
  ...
  async loadAbstractions(functionName) {
    ...
    let loadedSimulator = lodash.cloneDeep(
      await this.hgetAsync(functionName, "Abstraction")
    );
...
function makeGraphFromAbstractionList(
  abstractionList,
  passedGraph,
  returnAbstraction
) {
  ...
  for (const abstraction of abstractionList) {
    ...
    if (abstraction.InnerAbstractions.length > 0) {
      ...
      addPreviousNodeEdges(
        previousNode,
        firstInnerAbstraction,
        abstractionGraph
      );

      const ifSubGraph = makeGraphFromAbstractionList(
        abstraction.InnerAbstractions,
        abstractionGraph,
        true
      );
    ...
    if (
      abstraction.ElseAbstractions != null &&
      abstraction.ElseAbstractions.length > 0
    ) {
      ...
      addPreviousNodeEdges(
        previousNode,
        firstElseAbstraction,
        abstractionGraph
      );

      const elseSubGraph = makeGraphFromAbstractionList(
        abstraction.ElseAbstractions,
        abstractionGraph,
        true
      );
    ...
    if (abstraction.Paths != null && abstraction.Paths.length > 0) {
      ...
      functionPaths.forEach((path) => {
        if (
          path != null &&
          path.Abstractions != null &&
          path.Abstractions.length > 0
        ) {
          const firstPathAbstraction = path.Abstractions[0];
          addPreviousNodeEdges(
            previousNode,
            firstPathAbstraction,
            abstractionGraph
          );

          const pathLastNode = makeGraphFromAbstractionList(
            path,
            abstractionGraph,
            true
          );
...
function Identify_MainLoopBlock(controlFlowGraph) {
  ...
  const traverseObject = controlFlowGraph.traverseBFS(
    controlFlowGraph.FirstVertex,
    true
  );
...
function Identify_HashEachBlock(
  controlFlowGraph,
  blocksizes,
...
function Identify_StoreRemainMessage(
  controlFlowGraph,
  hashEachBlock,
  variable_definitions
) {
...
function Identify_PadLastPartialBlock(
  controlFlowGraph,
  storeRemainMessage,
  variable_definitions
) {
  ...
  storeRemainMessage.forEach((remainMsg) => {
    ...
    while (nextVal.value) {
      ...
      if (n.AbstractionType == "IF") {
        ...
        if (
          Is_End_Of_Block(n, tmp_buf, tmp_blk_aliases, affectedVars, BLOCKSIZE)
        ) {
          ...
          if (mem_stmts.length > 0) {
            for (const mem_stmt of mem_stmts) {
              ...
              if (
                (lodash.isEqual(tmp_buf, arg1) ||
                  Operation_Contains_Variable(arg1, tmp_buf)) &&
...
function Get_Lastdef(
  variableOperand,
  variableDefinitions,
  lineNumberOfOperation
) {
...
function Get_MemCpy_Stmt(ifAbstraction, controlFlowGraph, isMemSet = false) {
  ...
  const traverseGraph = controlFlowGraph.traverseDFSArrayOnly(
    ifAbstraction,
    true
  );
  let nextVal = traverseGraph.next();
...
function Is_More_Blocks(
  ...
  const conditionConstant = condition.lValue.isIMM()
    ...
    : condition.rValue != null && condition.rValue.isIMM()
    ? condition.rValue.value
    : null;
...
function Is_End_Of_Block(
  ...
  const targetCondition =
    hasTargetBuf &&
    ((hasLT && hasTargetNum) || (hasGT && hasZero) || (hasEq && hasZero));
...
function Is_Sub_Operation(
  ...
  while (!unwindRem_msg.isEmpty()) {
    ...
    if (op.isOP()) {
      ...
      const aliasIterator = IterateOverParameterAliases(
        op,
        rem_msg_abstraction
      );
...
function Has_Sub_Operation(operation, BLOCKSIZE) {
  ...
  const blocksizes = [
    BLOCKSIZE,
    BLOCKSIZE < 100 ? BLOCKSIZE * 8 : BLOCKSIZE / 8,
  ];
...
function postProcessAllFunctions() {
  for (const functionName of Object.keys(ProgramDetails.Functions)) {
    abstractionPostProcessing(
      ProgramDetails.Functions[functionName].Simulator.Abstractions
    );
  }
...
async function storeAllFunctionsInDB() {
  ...
  for (const functionName of Object.keys(ProgramDetails.Functions)) {
    if (
      ...
      await DB.storeAbstractions(
        functionName,
        ProgramDetails.Functions[functionName].Simulator
      );
      ProgramDetails.Functions[functionName].Simulator = null;
    }
  }
...
function abstractionPostProcessingR(abstractionList) {
  ...
  for (let i = 0; i < abstractionList.length; i++) {
    ...
    if (abstraction.AbstractionType == "IF") {
      ...
      for (var j = 0; j < abstraction.InnerAbstractions.length; j++) {
        ...
        if (innerAbstraction.AbstractionType == "FOR") {
          ...
          for (let k = 0; k < innerAbstraction.InnerAbstractions.length; k++) {
            const twoLevelsInnerAbstraction =
              innerAbstraction.InnerAbstractions[k];
          ...
          if (store8AbstractionIndex != null && ifAbstractionIndex != null) {
            ...
            const store8Abstraction =
              innerAbstraction.InnerAbstractions[store8AbstractionIndex];
            const ifAbstraction =
              innerAbstraction.InnerAbstractions[ifAbstractionIndex];
            ...
            const newMemCpyAbstraction = new MemCpyAbstraction(
              store8Abstraction
            );
            newMemCpyAbstraction.Size = newMemcpySize;

            // Insert Into For Abstraction
            innerAbstraction.InnerAbstractions.splice(
              store8AbstractionIndex,
              1,
              newMemCpyAbstraction
            );

            // Insert SET statement making top IF true
            if (ifAbstraction.Condition.isOP()) {
              const ifConditionSymbolicValue =
                ifAbstraction.Condition.getSymbolicValue();
              if (ifConditionSymbolicValue != null) {
                const varName = ifConditionSymbolicValue.name;
                const newSetAbstraction = new SetAbstraction(
                  varName,
                  ifAbstraction.Condition.lValue
                );
      ...
      if (
        store8AbstractionIndex != null &&
        ifAbstractionIndex != null &&
        forIndex != null
      ) {
        // Remove FOR Abstraction and move FOR Inner Abstractions to IF
        const newIfInnerAbstractionsList =
          abstraction.InnerAbstractions[forIndex].InnerAbstractions;
        abstraction.InnerAbstractions.splice(
          forIndex,
          1,
          ...newIfInnerAbstractionsList
        );
      }
    }
...
function abstractionPostProcessing(abstractionList) {
  RecurseOverInnerAbstractionsIteratively(
    abstractionList,
    () => {},
    abstractionPostProcessingR
  );
}

function RecurseOverInnerAbstractionsIteratively(
  abstractionList,
  abstractionCallback,
  abstractionListCallback
) {
  ...
  while (abstractionListStack.length > 0) {
    ...
    for (const innerAbs of currentList) {
      ...
      if (
        innerAbs.ElseAbstractions != null &&
        innerAbs.ElseAbstractions.length > 0
      ) {
...
function LinkFunctionCallR(abstraction, currentDepth = 0) {
  if (abstraction.AbstractionType == "CALL") {
    if (currentDepth < 4) {
      ...
      const loadedSimulator = lodash.cloneDeep(
        ProgramDetails.Functions[abstraction.FunctionName].Simulator
      ); // await DB.loadAbstractions(abstraction.FunctionName);
      if (
        loadedSimulator.Abstractions != null &&
        loadedSimulator.Abstractions.length > 0
      ) {
...
function onLineReadForDetails(line) {
  ...
  while (currentTokenIndex < tokens.length) {
    ...
    if (
      token == TOKENS.DATA_TOKEN &&
      tokens[currentTokenIndex + 2] == TOKENS.i32_const
    ) {
    ...
    if (
      token == TOKENS.IMPORT_TOKEN &&
      tokens.includes(TOKENS.FUNCTION_TOKEN)
    ) {
      let funcTokenIndex = tokens.indexOf(TOKENS.FUNCTION_TOKEN);
      let typeTokenIndex = tokens.indexOf(TOKENS.FUNCTION_TYPE_TOKEN);
      const importName = tokens
        .slice(currentTokenIndex + 1, funcTokenIndex)
        .join(" ");
      const importType = TOKENS.FUNCTION_TOKEN;
      const importFunctionType =
        ProgramDetails.Types[tokens.slice(typeTokenIndex + 1).join("")];
    ...
    if (
      token == TOKENS.EXPORT_TOKEN &&
      tokens.includes(TOKENS.FUNCTION_TOKEN)
    ) {
      let funcTokenIndex = tokens.indexOf(TOKENS.FUNCTION_TOKEN);
      const exportName = tokens
        .slice(currentTokenIndex + 1, funcTokenIndex)
        .join(" ");
    ...
    if (
      ...
      if (ProgramDetails.Functions[functionName] == null) {
        ...
        ProgramDetails.Functions[functionName].NumberOfLoops =
          functionDetails.NumberOfLoops;
        ProgramDetails.Functions[functionName].NumberOfIfs =
          functionDetails.NumberOfIfs;
        ProgramDetails.Functions[functionName].Loops = functionDetails.Loops;
        ProgramDetails.Functions[functionName].LoopLines =
          functionDetails.LoopLines;
        ProgramDetails.Functions[functionName].Ifs = functionDetails.Ifs;
        ProgramDetails.Functions[functionName].HasResult =
          functionDetails.HasResult;
        ProgramDetails.Functions[functionName].ResultType =
          functionDetails.ResultType;
        ProgramDetails.Functions[functionName].FunctionType =
          functionDetails.FunctionType;
      }
    ...
    if (
      token == TOKENS.FUNCTION_TOKEN &&
      tokens[currentTokenIndex + 1] == TOKENS.EXPORT_TOKEN
    ) {
      const functionName = tokens[currentTokenIndex + 1];
      // Update current function context
      currentFunction = functionName;
      ...
      case TOKENS.call:
        {
          ...
          if (ProgramDetails.Functions[functionCallName] == null) {
            ProgramDetails.Functions[functionCallName] = new FunctionDetails(
              functionCallName
            );
          }
...
async function onLineReadForDetailsEnd() {
  ...
  totalMemoryUsage +=
    afterFunctionPostProcess.heapUsed - beforeFunctionPostProcess.heapUsed;
          ...
          DB.loadAbstractions(functionName)
            .then((loadedSimulator) => {
              ...
              if (
                ...
                let graph = makeGraphFromAbstractionList(
                  loadedSimulator.Abstractions
                );
                ...
                totalMemoryUsage +=
                  afterMemUsage.heapUsed - beforeMemUsage.heapUsed;
                const variable_definitions =
                  loadedSimulator.VirtualStates[
                    loadedSimulator.VirtualStates.length - 1
                  ][0];
                ...
                const StoreRemainMessage = Identify_StoreRemainMessage(
                  graph,
                  HashEachBlock
                );
                const PadLastPartialBlock = Identify_PadLastPartialBlock(
                  graph,
                  StoreRemainMessage
                );
